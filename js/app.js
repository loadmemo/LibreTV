        ...
        apiItem.innerHTML = `
            <div class="flex items-center flex-1 min-w-0">
                <input type="checkbox" id="custom_api_${index}" 
                       class="form-checkbox h-3 w-3 text-[#8B5CF6] mr-1" 
...
function toggleSourceFilter() {
    ...
    const icon = document.getElementById('sourceFilterIcon');
...
// 渲染来源筛选栏
function renderSourceFilters(results) {
    if (!results || results.length === 0) return;

    const sourceMap = {};
    results.forEach(item => {
        const name = item.source_name || '未知来源';
        sourceMap[name] = (sourceMap[name] || 0) + 1;
    });

    const sources = Object.entries(sourceMap).sort((a, b) => b[1] - a[1]);
    const toggleDiv = document.getElementById('sourceFilterToggle');
    const filtersDiv = document.getElementById('sourceFilters');
...
function renderResultsGrid(results) {
    ...
    if (activeSourceFilter !== 'all') {
        results = results.filter(r => (r.source_name || '未知来源') === activeSourceFilter);
    }
...
<svg class="mx-auto h-12 w-12 text-[#808080]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        activeSourceFilter = 'all';
        renderSourceFilters(allResults);
